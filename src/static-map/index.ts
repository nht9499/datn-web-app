...
export const staticMapUrl = (props: AnyObject) => {
  const {
    markers,
    markerGroups,
    paths,
...
export const asyncStaticMapUrl = (props: AnyObject, requestStrategy = 'native') => {
  return new Promise((resolve, reject) => {
    ...
    const childrenUrlParts: AnyObject[] = [];

    if (Array.isArray(directions) && directions.length) {
      Promise.all(directions.map(
        direction => directionStrategy(direction, mapProps, requestStrategy as any)
      ))
        .then((directionUrlParts: any) => {
