<template>
  <div :id="wrapperId" class="simple-typeahead">
    <q-input
      ...
      v-bind="$attrs">
      <template #append>
        <q-spinner v-show="loading" size="sm" color="primary" />
      </template>
    </q-input>
    <div v-if="isListVisible" class="simple-typeahead-list">
      ...
      <q-list bordered>
        <q-item
          ...
          <q-item-section>
            <q-item-label
              v-if="item.isUserLocation"
              class="text-caption text-dark text-weight-bold">
              Vị trí của khách hàng
            </q-item-label>
            <q-item-label class="text-caption text-dark" lines="2">
              <span
                class="simple-typeahead-list-item-text"
                :data-text="itemProjection(item.description)"
                v-if="$slots['list-item-text']">
                <slot
                  name="list-item-text"
                  :item="item"
                  :item-projection="itemProjection"
                  :bold-match-text="boldMatchText"></slot>
              </span>
              <span
                class="simple-typeahead-list-item-text"
                :data-text="itemProjection(item.description)"
                v-html="boldMatchText(itemProjection(item.description))"
                v-else></span>
            </q-item-label>
          </q-item-section>
          <q-item-section side>
            <q-icon v-if="item.isSaved" name="bookmark" size="sm" color="orange" />
            <q-icon v-else-if="item.isUserLocation" name="gps_fixed" size="sm" color="negative" />
            <q-icon v-else name="place" size="sm" color="primary" />
          </q-item-section>
        </q-item>
      </q-list>
...
export default defineComponent({
  ...
  emits: ['onInput', 'onFocus', 'onBlur', 'selectItem', 'searchItem'],
  data() {
    return {
  ...
  methods: {
    ...
    selectCurrentSelection() {
      if (this.currentSelection) {
        ...
        this.$emit('searchItem', this.input)
      }
    },
